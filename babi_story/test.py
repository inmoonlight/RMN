...
def run(config):
    ...
    [test_q, test_a, test_a_num, test_c, test_l, test_c_real_len, test_q_real_len,
     test_hint], word_set = read_data_test(config['babi_processed'])
    num = len(test_c)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            batch_test = batch_iter_test(c=test_c, q=test_q, l=test_l, a=test_a, a_num=test_a_num,
                                         c_real_len=test_c_real_len, q_real_len=test_q_real_len,
                                         batch_size=config.batch_size)
            for test in batch_test:
                c_batch, q_batch, l_batch, a_batch, a_num_batch, c_real_len_batch, q_real_len_batch = zip(*test)
                feed_dict = {context: c_batch,
                             ...
                             is_training: False}
                correct_test, pred_test, alpha_test_1, alpha_test_2 = sess.run([correct, prediction, alpha_1, alpha_2],
                                                                               feed_dict=feed_dict)
    ...
    for task_result, task_num in zip(splited_result, tasks):
        ...
        for vc, va1, va2 in zip(task_result.Context.apply(lambda x: x.split("@")),
                                task_result.alpha_1.apply(lambda x: [float(y) for y in x.split("@")]),
                                task_result.alpha_2.apply(lambda x: [float(y) for y in x.split("@")])):
            visual_c.extend(vc)
            visual_a1.extend(va1)
            visual_a2.extend(va2)
        final_result = task_result[task_result.columns[3:]].loc[
            np.repeat(task_result.index, [config['c_max_len']] * task_result.shape[0])]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--metagraph', '--metagraph', help='path to the .meta')
